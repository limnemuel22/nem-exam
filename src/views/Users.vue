<template>
  ...
  <div class="users" v-if="users.length > 0">
    <div class="container col-md-12 add-form mt-5" v-if="isFormShow">
      ...
      <form>
        <div class="row text-left">
          ...
          <div class="col-md-12">
            <button type="submit" class="btn btn-secondary pull-right" @click="cancel">
              Cancel
            </button>
            <button type="submit" class="btn btn-success mr-2 pull-right" @click="submit">
              Submit
            </button>
          </div>
        </div>
  ...
  methods: {
    ...
    submit(e) {
      ...
      if (!this.currentId || this.currentId === "") {
        console.log(this.validateName(this.currentName));
        if (!this.validateName(this.currentName)) {
          return (this.nameErrorMessage = "Name is not valid!");
        }
        if (!this.validateEmail(this.currentEmail)) {
          return (this.emailErrorMessage = "Email is not valid!");
        }
        if (!this.validateUrl(this.currentWebsite)) {
          return (this.websiteErrorMessage = "Website is not valid");
        }
